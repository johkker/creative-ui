...
import { analyzeSourceForSlop, buildCritiqueReport, designDNASchema, generateDesignBrief, generateUnslopPlan, type DesignDNA } from "@creative-ui/core";
...
export async function saveDNA(dna: DesignDNA, cwd = process.cwd()) {
  const valid = designDNASchema.parse(dna);
  const dir = join(cwd, ".creative-ui");
  await mkdir(dir, { recursive: true });
  await Promise.all([
    writeFile(join(dir, "design-dna.json"), `${JSON.stringify(valid, null, 2)}\n`),
    writeFile(join(dir, "DESIGN.md"), generateDesignBrief(valid)),
  ]);
}
...
async function walk(root: string, current: string, files: string[], maxFiles: number) {
  ...
  for (const entry of await readdir(current)) {
    ...
    if (info.isDirectory()) { if (!ignoredDirectories.has(entry)) await walk(root, full, files, maxFiles); continue; }
...
export async function collectProjectSource(target = process.cwd(), maxFiles = 500): Promise<{ source: string; files: string[] }> {
  ...
  if (info.isDirectory()) await walk(root, target, files, maxFiles); else files.push(target);
  const chunks: string[] = [];
  for (const file of files) {
    try { const raw = await readFile(file, "utf8"); chunks.push(`\n/* creative-ui:file ${relative(root, file)} */\n${raw}`); }
    catch { /* Best-effort critique: ignore unreadable source files. */ }
  }
